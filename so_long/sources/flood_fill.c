...
char **sl_breadth_first_search(char **content, int x, int y)
{
    if (content[y][x] != '1')
    {
        if (content[y][x] == '0' || content[y][x] == 'P' || content[y][x] == 'E' || content[y][x] == 'C' || content[y][x] == 'F')
            content[y][x] = '1';
        else
            return (content);
...
bool sl_check_floodfill_prob(t_map *my_map)
{
    ...
    map_after_ff = sl_breadth_first_search(my_map->dup_content, my_map->player_x, my_map->player_y);
